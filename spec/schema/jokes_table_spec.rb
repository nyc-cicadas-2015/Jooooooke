...
describe "joke table" do

  it "jokes table in the database" do
    database_connection = ActiveRecord::Base.connection
    expect(database_connection.table_exists? :jokes).to be_truthy, "Expected the database to have a jokes table"
  end
  ...
  begin
    actual_columns = ActiveRecord::Base.connection.columns(:jokes)
  rescue
    actual_columns = []
  end
  ...
  expected_column_types_and_names.each do |expected_type, expected_column_names|
    expected_column_names.each do |expected_column_name|
      it "has a #{expected_type} column named #{expected_column_name}" do
        ...
        expect(matching_column).to be_truthy, "Did not find a #{expected_type} column named #{expected_column_name} in the joke table"
      end
    end
  end
end
